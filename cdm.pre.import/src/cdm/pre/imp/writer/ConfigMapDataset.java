...
public class ConfigMapDataset extends BaseElement {
   private String endItem;
   private Writer lstWriter;

   public ConfigMapDataset(final XMLStreamWriter streamWriter, final Writer lstWriter, final Element element, final String endItem) {
   ...
   /*
    *  Need to move the calculations of Dataset Object Name & file location to the configurable mapping. - Krishna/ Amit(non-Javadoc)
    *  
    *  now it is default way all the calculation are done inside the write method. it should be moved to Config mapping Utilities so that we can configure through the config map xml file.
    *  
    *  
    * @see cdm.pre.imp.writer.BaseElement#writeObjectValues(boolean)
    */
   ...
   public void writeObjectValues(final boolean hasChanges) throws XMLStreamException, IOException {
      ...
      writeValue("s", "ud", ICustom.ObjectName, objectName);
      ...
      writeValue("s", "ud", "file", location);
      
      HashMap<String, ArrayList<MappedAttributes>> mappedElementMap = element.getMappedElementsMap();
 	 if( mappedElementMap != null && mappedElementMap.size() > 0)
 	 {
 	
 			String tagDataset = WriterUtils.getAttributeValue(element, "tagDataset");
 			if( tagDataset != null && !tagDataset.equals(""))
 			{
 				writeValue("s", "ud", "tagDataset", tagDataset);
 			}
